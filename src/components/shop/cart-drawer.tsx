...
export function CartDrawer() {
  ...
  return (
    <>
      <div
        className="fixed inset-0 z-40 bg-[rgba(29,23,33,0.34)] backdrop-blur-[6px]"
        onClick={closeCart}
      ...
      <aside
        className="fixed right-0 top-0 z-50 flex h-dvh w-full max-w-[440px] flex-col overflow-hidden border-l border-white/50 bg-white/88 shadow-[0_20px_80px_rgba(23,18,28,0.24)] backdrop-blur-2xl sm:max-w-[470px]"
        role="dialog"
        aria-modal="true"
        aria-label="Shopping cart"
        style={{ animation: 'cartSlideIn 420ms cubic-bezier(0.16,1,0.3,1) both' }}
      >
        <div className="relative overflow-hidden border-b border-dusk/8 px-5 py-5 sm:px-6">
        ...
        {items.length === 0 ? (
          ...
          <>
            <div className="flex-1 overflow-y-auto px-5 py-5 sm:px-6">
              <div className="rounded-[1.6rem] border border-dusk/8 bg-[linear-gradient(160deg,rgba(244,220,232,0.5),rgba(255,255,255,0.98),rgba(243,233,255,0.7))] p-4 shadow-soft">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="inline-flex h-10 w-10 items-center justify-center rounded-full bg-white/75 text-plum shadow-soft">
                    <Truck size={18} />
                  </div>
                </div>
              ...
              <div className="mt-5 space-y-4">
                {items.map((item, index) => (
                  <article
                    key={item.id}
                    className="group rounded-[1.5rem] border border-dusk/8 bg-white p-3 shadow-[0_10px_28px_rgba(33,28,26,0.05)] transition duration-300 hover:-translate-y-0.5 hover:shadow-[0_16px_34px_rgba(33,28,26,0.08)]"
                    style={{
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-start justify-between gap-3">
                          <div className="min-w-0">
                            ...
                            {item.variant ? (
                              <p className="mt-1 text-[11px] uppercase tracking-[0.18em] text-dusk/42">
  ...
  .join(' • ')}
            ...
            <div className="border-t border-dusk/8 bg-white/92 px-5 py-5 sm:px-6">
              ...
              <div className="mt-4 space-y-2 text-sm">
                ...
                <div className="flex items-center justify-between border-t border-dusk/8 pt-3">
                  ...
                  <span className="font-display text-3xl text-dusk">
              ...
              <div className="mt-5 space-y-3">
                <Link
                  href="/checkout"
                  onClick={closeCart}
                  className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-dusk px-6 py-3.5 text-sm font-semibold text-white transition duration-500 hover:-translate-y-0.5 hover:bg-plum hover:shadow-[0_14px_34px_rgba(91,45,142,0.22)]"
                >
                  Proceed to checkout
                  <ArrowRight size={16} />
                </Link>
                ...
                <button
                  onClick={closeCart}
                  className="inline-flex w-full items-center justify-center rounded-full border border-dusk/10 bg-white px-6 py-3 text-sm font-medium text-dusk/70 transition duration-300 hover:border-plum/20 hover:bg-pearl hover:text-plum"
                >
                  Continue shopping
                </button>
              </div>
